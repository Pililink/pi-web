...
const PROVIDER_ICONS: Record<string, { Icon: IconComponent; hasColor: boolean }> = {
  ...
  reqtoken: { Icon: OpenAIIcon, hasColor: false },
...
function ProviderFallback({ size }: { size: number }) {
  return (
    <svg
      width={size}
      height={size}
...
/** Renders a provider logo, falling back to a neutral processor icon. */
export function ProviderIcon({ id, size = 14 }: { id: string; size?: number }) {
  const providerIcon = PROVIDER_ICONS[id.toLowerCase()];
  if (!providerIcon) return <ProviderFallback size={size} />;
