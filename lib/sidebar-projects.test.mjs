...
test("groups linked worktrees by project root and sorts by latest session activity", async () => {
  ...
  // Without an explicit order, new roots keep discovery order (first seen),
  // not pure recency ranking.
...
test("reorderIds and mergeSessionOrder preserve manual arrangement", async () => {
  const {
  ...
  assert.deepEqual(reorderIds(["a", "b", "c"], "a", "b", "before"), ["a", "b", "c"]);
  assert.deepEqual(reorderIds(["a", "b", "c"], "c", "b", "after"), ["a", "b", "c"]);
  assert.deepEqual(mergeSessionOrder(["b", "a"], ["c", "a", "b"]), ["c", "b", "a"]);

  const tree = buildSidebarSessionTree([
    session({ id: "a", modified: "2026-07-21T12:00:00.000Z" }),
    session({ id: "b", modified: "2026-07-21T11:00:00.000Z" }),
    session({ id: "c", modified: "2026-07-21T10:00:00.000Z" }),
  ]);
  const ordered = applySessionOrderToTree(tree, ["c", "a", "b"]);
  assert.deepEqual(ordered.map((node) => node.session.id), ["c", "a", "b"]);
});

test("stable projectOrder keeps cleared projects in place and appends new roots", async () => {
  const {
  ...
  const order = mergeProjectOrder([], ["/repos/a", "/repos/b", "/repos/c"]);
  assert.deepEqual(order, ["/repos/a", "/repos/b", "/repos/c"]);

  // Clear project b: pin as manual with a very recent lastOpened.
  const manual = upsertManualProject(
    [{ root: "/repos/a", lastOpened: "2026-07-20T00:00:00.000Z" }],
    "/repos/b",
    "2099-01-01T00:00:00.000Z",
  );
  // Upsert should not move existing manual entry to the front.
  assert.deepEqual(manual.map((item) => item.root), ["/repos/a", "/repos/b"]);

  const afterClear = groupSidebarProjects(
    [
      session({ id: "a", projectRoot: "/repos/a", cwd: "/repos/a", modified: "2026-07-20T01:00:00.000Z" }),
      session({ id: "c", projectRoot: "/repos/c", cwd: "/repos/c", modified: "2026-07-20T02:00:00.000Z" }),
    ],
    manual,
    order,
  );
  assert.deepEqual(afterClear.map((group) => group.root), ["/repos/a", "/repos/b", "/repos/c"]);
  assert.equal(afterClear[1].sessions.length, 0);

  // Brand-new root is appended, not prepended.
  const withNew = mergeProjectOrder(order, ["/repos/a", "/repos/b", "/repos/c", "/repos/d"]);
  assert.deepEqual(withNew, ["/repos/a", "/repos/b", "/repos/c", "/repos/d"]);
});

test("mergeProjectOrder prunes missing roots — callers must not pass partial sets", async () => {
  const { mergeProjectOrder, reorderIds, groupSidebarProjects } = await loadSubject();

  // User drag-ordered: avatar moved to the end.
  const saved = ["/repos/avatar-client-2.5d", "/repos/a", "/repos/b"];
  const reordered = reorderIds(saved, "/repos/avatar-client-2.5d", "/repos/b", "after");
  assert.deepEqual(reordered, ["/repos/a", "/repos/b", "/repos/avatar-client-2.5d"]);

  // Bug pattern: before sessions hydrate, only manual roots are "visible".
  // Pruning against that partial set wipes the drag order permanently.
  const partialManualOnly = mergeProjectOrder(reordered, ["/repos/avatar-client-2.5d"]);
  assert.deepEqual(partialManualOnly, ["/repos/avatar-client-2.5d"]);

  // After a bad prune, full hydrate can only append the rest — avatar jumps to top.
  const afterBadPrune = mergeProjectOrder(partialManualOnly, [
    "/repos/a",
    "/repos/b",
    "/repos/avatar-client-2.5d",
  ]);
  assert.deepEqual(afterBadPrune, ["/repos/avatar-client-2.5d", "/repos/a", "/repos/b"]);

  // Correct pattern: never prune until full roots are known; full merge keeps drag order.
  const afterGoodMerge = mergeProjectOrder(reordered, [
    "/repos/a",
    "/repos/b",
    "/repos/avatar-client-2.5d",
  ]);
  assert.deepEqual(afterGoodMerge, ["/repos/a", "/repos/b", "/repos/avatar-client-2.5d"]);

  const groups = groupSidebarProjects(
    [
      session({ id: "a", projectRoot: "/repos/a", cwd: "/repos/a", modified: "2026-07-20T01:00:00.000Z" }),
      session({ id: "b", projectRoot: "/repos/b", cwd: "/repos/b", modified: "2026-07-20T02:00:00.000Z" }),
      session({
        id: "avatar",
        projectRoot: "/repos/avatar-client-2.5d",
        cwd: "/repos/avatar-client-2.5d",
        modified: "2026-07-20T03:00:00.000Z",
      }),
    ],
    [{ root: "/repos/avatar-client-2.5d", lastOpened: "2026-07-21T00:00:00.000Z" }],
    reordered,
  );
  assert.deepEqual(groups.map((group) => group.root), [
    "/repos/a",
    "/repos/b",
    "/repos/avatar-client-2.5d",
  ]);
...
test("removeManualProject drops only the matching root", async () => {
  const { removeManualProject } = await loadSubject();
  assert.deepEqual(
    removeManualProject(
      [
        { root: "/repos/a", lastOpened: "2026-08-03T12:00:00.000Z" },
        { root: "/repos/b", lastOpened: "2026-08-03T11:00:00.000Z" },
      ],
      "/repos/a",
    ),
    [{ root: "/repos/b", lastOpened: "2026-08-03T11:00:00.000Z" }],
...
test("keeps empty manual projects after sessions are cleared and normalizes path separators", async () => {
  const { groupSidebarProjects, upsertManualProject } = await loadSubject();
  const pinned = upsertManualProject([], "D:\\src\\meetingtwin-backend\\", "2026-08-03T12:00:00.000Z");
  assert.deepEqual(pinned, [{
    root: "D:/src/meetingtwin-backend",
    lastOpened: "2026-08-03T12:00:00.000Z",
  }]);

  const groups = groupSidebarProjects([], pinned);
  assert.equal(groups.length, 1);
  assert.equal(groups[0].root, "D:/src/meetingtwin-backend");
  assert.equal(groups[0].sessions.length, 0);
  assert.equal(groups[0].manual, true);
});

test("detects temporary temp-session projects and partitions them from regular projects", async () => {
  const {
  ...
  assert.equal(isTemporaryProjectRoot("/home/u/.pi/agent/temp-session"), true);
  assert.equal(isTemporaryProjectRoot("/home/u/.pi/agent/temp-session/2026-08-05/f"), true);
  assert.equal(isTemporaryProjectRoot("C:\\Users\\u\\.pi\\agent\\temp-session\\2026-08-05\\f-2"), true);
  assert.equal(isTemporaryProjectRoot("/home/u/pi-cwd-20260803"), false);
  assert.equal(isTemporaryProjectRoot("/repos/pi-web"), false);
  assert.equal(getProjectDisplayName("/repos/SoulShell-FuncPilot"), "SoulShell-FuncPilot");

  const partitioned = partitionSidebarProjects([
    { root: "/repos/pi-web", sessions: [session({ id: "p1" })], latestActivity: "2026-08-03T12:00:00.000Z", manual: false },
    { root: "/home/u/.pi/agent/temp-session/2026-08-05/f", sessions: [session({ id: "t-new", modified: "2026-08-05T13:00:00.000Z" })], latestActivity: "2026-08-05T13:00:00.000Z", manual: true },
    { root: "/home/u/.pi/agent/temp-session/2026-08-05/f-2", sessions: [session({ id: "t-old", modified: "2026-08-05T12:00:00.000Z" })], latestActivity: "2026-08-05T12:00:00.000Z", manual: true },
  ]);

  assert.deepEqual(partitioned.projects.map((group) => group.root), ["/repos/pi-web"]);
  assert.deepEqual(partitioned.temporary.map((group) => group.root), [
    "/home/u/.pi/agent/temp-session/2026-08-05/f",
    "/home/u/.pi/agent/temp-session/2026-08-05/f-2",
  ]);
  assert.deepEqual(
    flattenTemporarySessions(partitioned.temporary).map((item) => item.id),
    ["t-new", "t-old"],
  );
});
