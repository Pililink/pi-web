...
test("Side Chat uses the main prompt and exposes the upstream peek parameters", async () => {
  ...
  assert.match(promptResult.systemPrompt, /You are in a side conversation, not the main thread/);
  assert.match(promptResult.systemPrompt, /inherited fork history is provided only as reference context/);
  assert.match(promptResult.systemPrompt, /Sub-agents are off-limits in this side conversation/);
  assert.match(promptResult.systemPrompt, /Do not modify files, source, git state/);
  assert.match(promptResult.systemPrompt, /use `peek_main`/);
