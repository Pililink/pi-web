...
test("Refork and Clear abort an active side run and inherit current main preferences", async () => {
  ...
  const openSource = source.slice(
    source.indexOf("export async function openSideChat"),
    source.indexOf("export async function setSideChatToolMode"),
  );
  const modeSource = source.slice(source.indexOf("export async function setSideChatToolMode"));

  assert.match(openSource, /currentWrapper\.send\(\{ type: "abort" \}\)/);
  assert.match(openSource, /getSessionPreferences\(mainManager\)/);
  assert.match(openSource, /action === "open" \? currentMarker\?\.toolMode \?\? "readonly" : "readonly"/);
  ...
  assert.doesNotMatch(modeSource, /session\.isRunning\(\)/);
});
