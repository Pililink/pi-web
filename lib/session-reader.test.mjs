...
function sideChatMarker(id, parentId) {
  return {
    type: "custom",
    id,
    parentId,
    timestamp: "2026-01-01T00:00:01.000Z",
    customType: "pi-web-side-chat",
    data: {
      mainSessionId: "main",
      status: "active",
      toolMode: "readonly",
...
test("marks inherited Side Chat messages as UI-hidden without removing model context", () => {
  const entries = [
    userEntry("u1", null, "main request"),
    assistantEntry("a1", "u1", "main answer"),
    sideChatMarker("side", "a1"),
    userEntry("u2", "side", "side question"),
    assistantEntry("a2", "u2", "side answer"),
  ];

  const context = buildSessionContext(entries);

  assert.deepEqual(context.entryIds, ["u1", "a1", "u2", "a2"]);
  assert.deepEqual(context.hiddenMessageEntryIds, ["u1", "a1"]);
  assert.equal(context.messages.length, 4);
});

test("hides all inherited messages before the first Side Chat prompt", () => {
  const entries = [
    userEntry("u1", null, "main request"),
    assistantEntry("a1", "u1", "main answer"),
    sideChatMarker("side", "a1"),
  ...
  assert.deepEqual(context.hiddenMessageEntryIds, ["u1", "a1"]);
  assert.equal(context.hiddenMessageEntryIds.length, context.messages.length);
});

test("keeps inherited Side Chat messages hidden across compaction ordering", () => {
  const entries = [
    userEntry("u1", null, "main request"),
    assistantEntry("a1", "u1", "main answer"),
    sideChatMarker("side", "a1"),
    userEntry("u2", "side", "side question"),
    {
      ...
      summary: "compacted Side Chat",
      firstKeptEntryId: "u1",
      tokensBefore: 123,
    },
    assistantEntry("a2", "cmp", "side answer"),
  ];

  const context = buildSessionContext(entries);

  assert.deepEqual(context.entryIds, ["cmp", "u1", "a1", "u2", "a2"]);
  assert.deepEqual(context.hiddenMessageEntryIds, ["u1", "a1"]);
});
