...
test("openOrFocus updates files chip title", async () => {
  const { openOrFocusRightPanelTab, updateRightPanelTabTitle } = await load();
  const opened = openOrFocusRightPanelTab([], "files", "README.md");
  assert.equal(opened.tabs[0].title, "README.md");
  const cleared = updateRightPanelTabTitle(opened.tabs, "files", null);
  assert.equal(cleared[0].title, undefined);
});
