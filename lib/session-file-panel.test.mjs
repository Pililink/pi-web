...
test("file surface with zero tabs collapses to closed on restore", async () => {
  ...
  assert.deepEqual(
    ...
    { open: false, surface: "home" },
  );
});

test("deriveRightPanelMode maps open+surface including home/review", async () => {
  ...
  assert.equal(deriveRightPanelMode({ open: true, surface: "file", tabCount: 1 }), "file");
  assert.equal(deriveRightPanelMode({ open: true, surface: "file", tabCount: 0 }), "home");
  assert.equal(deriveRightPanelMode({ open: true, surface: "home" }), "home");
  assert.equal(deriveRightPanelMode({ open: true, surface: "review" }), "review");
  assert.equal(deriveRightPanelMode({ open: true, surface: "sideChat" }), "chat");
});

test("resolveOpenRightPanelSurface prefers content then Codex home", async () => {
  const { resolveOpenRightPanelSurface } = await loadSubject();
  assert.equal(
    resolveOpenRightPanelSurface({ lastFilesSurface: "file", tabCount: 0, hasCwd: true }),
    "home",
  );
  assert.equal(
    resolveOpenRightPanelSurface({ lastFilesSurface: "explorer", tabCount: 0, hasCwd: true }),
    "explorer",
  );
  assert.equal(
    resolveOpenRightPanelSurface({ lastFilesSurface: "file", tabCount: 2, hasCwd: true }),
    "file",
  );
  assert.equal(
    resolveOpenRightPanelSurface({
      lastFilesSurface: "file",
      tabCount: 0,
      hasCwd: true,
      preferSideChat: true,
    }),
